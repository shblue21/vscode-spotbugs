    ...
    try {
      ...
      if (fileUri.fsPath.endsWith('.java') || fileUri.fsPath.endsWith('.class')) {
        try {
          ...
          if (classpathsResult && Array.isArray(classpathsResult.classpaths) && classpathsResult.classpaths.length > 0) {
...
export async function runWorkspaceAnalysis(config: Config, spotbugsTreeDataProvider: SpotbugsTreeDataProvider, treeView: TreeView<TreeItem>): Promise<void> {
  ...
  try {
    ...
    if (!javaExt) {
      ...
      Logger.log(`redhat.java present. Active=${javaExt.isActive}, Version=${(javaExt as any).packageJSON?.version ?? 'unknown'}`);
      if (!javaExt.isActive) {
        try {
          await javaExt.activate();
          Logger.log('Activated redhat.java extension.');
        } catch (e) {
          Logger.log(`Warning: Failed to activate redhat.java (${e instanceof Error ? e.message : String(e)})`);
        }
      }
      ...
      if (api && typeof api.serverReady === 'function') {
        try {
          ...
          Logger.log(`Warning: serverReady() failed (${e instanceof Error ? e.message : String(e)})`);
        }
      }
    }
    ...
    try {
      ...
      buildResult = await commands.executeCommand<number>(JavaLanguageServerCommands.BUILD_WORKSPACE, false);
    ...
    if (buildResult !== 0) {
      try {
        ...
        buildResult = await commands.executeCommand<number>(JavaLanguageServerCommands.BUILD_WORKSPACE, true);
    ...
    try {
      ...
      if (classpathsResult) {
        ...
        Logger.log(`Classpaths result - output:${classpathsResult.output ?? 'n/a'}, classpaths:${Array.isArray(cps) ? cps.length : 0}, sourcepaths:${Array.isArray(sps) ? sps.length : 0}`);
        ...
        if (!outputPath && Array.isArray(cps)) {
          outputPath = await pickCandidateOutputFolderFromClasspaths(cps, workspaceFolder.uri.fsPath);
          if (outputPath) {
...
async function getClasspathsWithFallback(preferredUri?: Uri): Promise<any | undefined> {
  ...
  try {
    ...
    if (cmds.includes(JavaLanguageServerCommands.GET_ALL_JAVA_PROJECTS)) {
      try {
        const uris = await commands.executeCommand<string[]>(JavaLanguageServerCommands.GET_ALL_JAVA_PROJECTS);
    ...
    try {
      ...
      if (param !== undefined) {
        try {
          ...
          Logger.log(`getClasspaths(${attempt.label}) direct failed: ${e instanceof Error ? e.message : String(e)}`);
        }
        if (!res) {
          try {
            res = await commands.executeCommand<any>(JavaLanguageServerCommands.GET_CLASSPATHS, param, 'runtime');
          } catch (e2) {
            Logger.log(`getClasspaths(${attempt.label}, runtime) failed: ${e2 instanceof Error ? e2.message : String(e2)}`);
          }
        }
      }
      if (!res) {
        try {
          ...
          Logger.log(`getClasspaths(no-arg within ${attempt.label}) failed: ${e3 instanceof Error ? e3.message : String(e3)}`);
        }
      }
      ...
      if (!res && api && typeof api.getClasspaths === 'function' && param) {
        try {
          ...
          if (cpRes) {
            ...
            Logger.log(`Using extension API getClasspaths for ${attempt.label}: classpaths=${Array.isArray(res.classpaths) ? res.classpaths.length : 0}`);
          }
        } catch (e4) {
          Logger.log(`extensionApi.getClasspaths(${attempt.label}) failed: ${e4 instanceof Error ? e4.message : String(e4)}`);
        }
      }
      if (res) {
        ...
        Logger.log(`getClasspaths(${attempt.label}) succeeded: output=${res.output ?? 'n/a'}, classpaths=${cps}, sourcepaths=${sps}`);
        return res;
      }
      Logger.log(`getClasspaths(${attempt.label}) returned empty result`);
    } catch (e) {
      Logger.log(`getClasspaths(${attempt.label}) failed: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
  return undefined;
}

async function pickCandidateOutputFolderFromClasspaths(classpaths: string[], workspacePath: string): Promise<string | undefined> {
  const jarsExcluded = classpaths.filter(p => !p.toLowerCase().endsWith('.jar') && !p.toLowerCase().endsWith('.zip'));
